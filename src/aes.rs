const INV_SBOX: [u8; 256] = [
    ,
];

const SBOX: [u8; 256] = [
    ,
];

const RCON: [u8; 11] = [0, 1, 2, 4, 8, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];

pub(crate) fn decrypt_ecb_pkcs7(key: &[u8; 16], data: &[u8]) -> Result<Vec<u8>, &'static str> {
    if data.is_empty() || !data.len().is_multiple_of(16) {
        return Err("AES 密文长度不是非零的 16 字节倍数");
    }
    let keys = expand_key(key);
    let mut out = data.to_vec();
    for block in out.chunks_exact_mut(16) {
        decrypt_block(block.try_into().expect("固定的 AES 分块"), &keys);
    }
    let pad = *out.last().expect("非空") as usize;
    if pad == 0
        || pad > 16
        || out[out.len() - pad..]
            .iter()
            .any(|&byte| byte as usize != pad)
    {
        return Err("AES PKCS#7 填充无效");
    }
    out.truncate(out.len() - pad);
    Ok(out)
}

fn expand_key(key: &[u8; 16]) -> [u8; 176] {
    let mut expanded = [0u8; 176];
    expanded[..16].copy_from_slice(key);
    let (mut generated, mut round) = (16, 1);
    let mut temp = [0u8; 4];
    while generated < expanded.len() {
        temp.copy_from_slice(&expanded[generated - 4..generated]);
        if generated % 16 == 0 {
            temp.rotate_left(1);
            for byte in &mut temp {
                *byte = SBOX[*byte as usize];
            }
            temp[0] ^= RCON[round];
            round += 1;
        }
        for byte in temp {
            expanded[generated] = expanded[generated - 16] ^ byte;
            generated += 1;
        }
    }
    expanded
}

fn decrypt_block(state: &mut [u8; 16], keys: &[u8; 176]) {
    add_round_key(state, &keys[160..176]);
    for round in (1..10).rev() {
        inv_shift_rows(state);
        for byte in state.iter_mut() {
            *byte = INV_SBOX[*byte as usize];
        }
        add_round_key(state, &keys[round * 16..(round + 1) * 16]);
        inv_mix_columns(state);
    }
    inv_shift_rows(state);
    for byte in state.iter_mut() {
        *byte = INV_SBOX[*byte as usize];
    }
    add_round_key(state, &keys[..16]);
}

fn add_round_key(state: &mut [u8; 16], key: &[u8]) {
    for (byte, key_byte) in state.iter_mut().zip(key) {
        *byte ^= key_byte;
    }
}

fn inv_shift_rows(s: &mut [u8; 16]) {
    let o = *s;
    s[1] = o[13];
    s[5] = o[1];
    s[9] = o[5];
    s[13] = o[9];
    s[2] = o[10];
    s[6] = o[14];
    s[10] = o[2];
    s[14] = o[6];
    s[3] = o[7];
    s[7] = o[11];
    s[11] = o[15];
    s[15] = o[3];
}

fn inv_mix_columns(s: &mut [u8; 16]) {
    for c in s.chunks_exact_mut(4) {
        let [a, b, d, e] = [c[0], c[1], c[2], c[3]];
        c[0] = mul(a, 14) ^ mul(b, 11) ^ mul(d, 13) ^ mul(e, 9);
        c[1] = mul(a, 9) ^ mul(b, 14) ^ mul(d, 11) ^ mul(e, 13);
        c[2] = mul(a, 13) ^ mul(b, 9) ^ mul(d, 14) ^ mul(e, 11);
        c[3] = mul(a, 11) ^ mul(b, 13) ^ mul(d, 9) ^ mul(e, 14);
    }
}

fn mul(mut a: u8, mut b: u8) -> u8 {
    let mut result = 0;
    while b != 0 {
        if b & 1 != 0 {
            result ^= a;
        }
        let high = a & 0x80;
        a <<= 1;
        if high != 0 {
            a ^= 0x1b;
        }
        b >>= 1;
    }
    result
}

#[cfg(test)]
mod tests {
    use super::{decrypt_block, expand_key};

    #[test]
    fn fips_197_decryption_vector() {
        let key = [
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
            0x0e, 0x0f,
        ];
        let mut block = [
            0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4,
            0xc5, 0x5a,
        ];
        decrypt_block(&mut block, &expand_key(&key));
        assert_eq!(
            block,
            [
                0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd,
                0xee, 0xff
            ]
        );
    }
}
